...
function genLineGraph(g::SimpleGraph{T}) where {T<:Integer}
    ...
    #> Group edges incident to each vertex in `g` so that edges in each group are adjacent
    adjacentEdgeGroups = Memory{Memory{typeE}}(undef, nv)
    for v in 1:nv
        adjacentEdgeGroups[begin+v-1] = Memory{typeE}(undef, vertexReg[begin+v-1])
        vertexReg[begin+v-1] = 0 #> Reset as a labeling counter for edges incident at `v`
    end
    #> Record each edge's line-graph vertex (by its index in `edges`) under its endpoints
    for (edgeIdx, (i, j)) in enumerate(edges)
        adjacentEdgeGroups[begin+i-1][begin + (vertexReg[begin+i-1] += 1) - 1] = edgeIdx
        adjacentEdgeGroups[begin+j-1][begin + (vertexReg[begin+j-1] += 1) - 1] = edgeIdx
    end

    #> Two edges are adjacent iff they share an endpoint (as their common incident vertex)
    for v in eachindex(adjacentEdgeGroups)
